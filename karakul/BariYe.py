...
?start: action
action: integer_container (variable_scalar|integer_container) glyphselector
"""
...
def get_xmax_variable(font, g):
    vs = VariableScalar()
    vs.axes = font.axes
    vs.values = {
        Location(l.master.location): (l.bounds or [0, 0, 0])[2]
        for l in font.glyphs[g].layers
        if l._master
    }
    return vs

def get_rsb_variable(font, g):
    vs = VariableScalar()
    vs.axes = font.axes
    vs.values = {
        Location(l.master.location): l.rsb
        for l in font.glyphs[g].layers
        if l._master
    }
    return vs

def variable_max(v1, v2):
    vs = VariableScalar()
    vs.axes = v1.axes
    for loc in v1.values.keys():
        vs.values[loc] = max(v1.values[loc], v2.values[loc])
    return vs
...
class BYMoveDots(FEZVerb):
    def action(self, args):
        ...
        below_dots_yb = list(sorted(filter(lambda x: x.endswith(".yb"), below_dots)))
        ...
        for bariye in parser.fontfeatures.namedClasses["bariye"]:
            ...
            bariye_tail = max(
                ...
                (get_xmax_variable(self.parser.font, bariye) - entry_anchor[0]).default,
            )
            warnings.warn(
                "Xmax: %s"
                % (get_xmax_variable(self.parser.font, bariye) - entry_anchor[0])
            )
            warnings.warn("BariYe tail: %s" % str(bariye_tail))
            ...
            while len(queue) > 0:
                ...
                warnings.warn(
                    "Sequence %s total %i bariye_tail %i"
                    % (repsequence, seq_length, bariye_tail)
                )

                if (
                    seq_length > bariye_tail
                ...
                if alwaysDrop:
                    ...
                    lu[0] = [
                        self.parser.fontfeatures.referenceRoutine(maybeDropDotRoutine)
                    ]
    ...
    def get_yb_clearance(self, parser, bariye):
        ...
        if not intersections:
            warnings.warn("Couldn't find an intersection for %s" % bariye)
            return 0
        i = intersections[-1]
        return i.point.y
        ...
        if hasattr(parser.fontfeatures, "anchors"):
            anchor1_y = statistics.mean(
                [
                    ...
                    and "_bottom" in parser.fontfeatures.anchors[x]
                ]
            )
...
class BYFixOverhang(FEZVerb):
    def action(self, args):
        overhang_padding, space_fudge, glyphs = args
        overhang_padding = overhang_padding.resolve_as_integer()
        space_fudge = space_fudge.resolve_as_integer()
        parser = self.parser
        ...
        for yb in overhangers:
            ...
            overhang = variable_max(
                get_rsb_variable(parser.font, yb) * -1,
                get_xmax_variable(parser.font, yb) - entry_anchor[0],
            )
            ...
            while workqueue:
                ...
                # Urgh
                if totalwidth > overhang.default or len(string) > failsafe_max_length:
                    continue

                adjustment = overhang - totalwidth + overhang_padding
                postcontext = [x[0] for x in string[:-1]] + [[yb]]
                input_ = string[-1]
                example = [input_[0][0]] + [x[0] for x in postcontext]
                warnings.warn(
                    "For glyphs in %s, overhang=%s totalwidth=%s adjustment=%s"
                    % (example, overhang, totalwidth, adjustment)
                )
                maxchainlength = max(maxchainlength, len(string))
                new_adjustment = copy.deepcopy(adjustment)
                ...
                if any(new_adjustment.values.values()) > 200:
                    new_adjustment.values = {
                        ...
                        for k,v in new_adjustment.values()
                    }
                    if not new_adjustment.does_vary:
                        new_adjustment = new_adjustment.default
                    rules.append(
                        fontFeatures.Positioning(
                            [input_[0]],

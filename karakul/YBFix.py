...
logger = logging.getLogger("NastaliqConnections")
...
class DetectAndSwapYB(FEZVerb):
    def action(self, args):
        ...
        self.dot_carriers = [
            x for x in medis_inits if "JIM" in x or "BE" in x or "TE" in x
        ]
        self.haydb_carriers = [x for x in medis_inits if "HAYC" in x]
        self.kasra_carriers = [
            x
            for x in medis_inits
            if not ("JIM" in x or "TE" in x or "BE" in x or "HAYC" in x)
        ]
        binned_glyphs = (
        ...
        self.ybs = [
            x for x in glyphs if x.endswith(".yb") or x.endswith(".yb.collides")
        ]
        sequences = []
        ...
        cycle_dot = fontFeatures.Routine(
            ...
            rules=[
                fontFeatures.Substitution(
                    [[x for x in self.ybs if x.endswith(".yb")]],
                    [[self.cycle(x) for x in self.ybs if x.endswith(".yb")]],
                )
            ],
        )
        result = []
    ...
    def possible_sequence(self, right, rdot, left, ldot):
        ...
        if "haydb" in ldot and "HAYC" not in left:
            return False
        if re.search(r"^[sdt]db", rdot) and (
            "JIM" not in right and "BE" not in right and "TE" not in right
        ):
            return False
        if re.search(r"^[sdt]db", ldot) and (
            "JIM" not in left and "BE" not in left and "TE" not in left
        ):
            return False
        return True
    ...
    def cycle(self, dot):
        if not dot.endswith(".collides"):
            return dot + ".collides"

    def get_contexts(self):
        rules = load_rules(
            "sources/build/rules.csv", self.parser.font.exportedGlyphs(), full=True
        )
        # possible = set([])
